# MambaIRv2 network architecture, adapted for standalone distribution.
#
# Derived from MambaIR / MambaIRv2 by Hang Guo et al., the official implementation at
# https://github.com/csguoh/MambaIR, which its authors release under the Apache License,
# Version 2.0 (https://www.apache.org/licenses/LICENSE-2.0).
#
# Modifications by LogoLabs (2026): the `to_2tuple` / `trunc_normal_` helpers and the
# `ARCH_REGISTRY` below are vendored so the file imports without `basicsr`; no
# architecture changes. Redistributed under the Apache License, Version 2.0, as upstream.
#
# Vendored into inkvec-comfyui from https://huggingface.co/Logolabs/inkvec-sr-001
# (revision a16b17451224c9a51a2d6bb94459d96d18eb5267, git blob e791553a). One further
# modification, LogoLabs (2026): the selective scan is imported from the sibling `scan`
# module instead of `mamba_ssm`, which uses mamba-ssm's CUDA kernel when it is importable
# and a pure-PyTorch scan otherwise. Nothing else in this file differs from the source.
import math
import numpy as np
import torch
...
# --- standalone shims (packaging) -------------------------------------------
# Upstream pulls to_2tuple/trunc_normal_ from basicsr and a registry decorator.
# Both utilities are plain timm/torch helpers and the registry is only used as a
# decorator, so the package vendors them instead of depending on all of basicsr.
...
from .scan import selective_scan_fn, selective_scan_ref  # noqa: E402
        ...
        for layer in self.layers:
            if self.use_checkpoint and self.training:
                x = torch.utils.checkpoint.checkpoint(
                    layer, x, x_size, params, use_reentrant=False
                )
            else:
